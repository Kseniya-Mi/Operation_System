...
int main()
{
	...
	string creator_exe = "Creator_part.exe " + binary_name + " " + amount_of_notes;
	...
	if (input_name.is_open()) {
		...
		string reporter_exe = "Reporter_part.exe " + binary_name + " " + txt_name + " " + pay_for_hour;
		...
		if (output_name.is_open()) {
			while (!output_name.eof()) {
				string temporary;
				getline(output_name, temporary); //ÄÎÁÀÂÈÒÜ ÎÁÐÀÁÎÒÊÓ ÈÑÊËÞ×ÅÍÈÉ
				cout << temporary << "\n";
			}
		}
		output_name.close();
